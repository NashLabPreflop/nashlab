...
function buildRandomSet(groupId, count, prevFirst = null, spotFilter = "all") {
  ...
  const spots = isLegacy ? [] : rawSpots.filter((spot) => spotMatchesFilter(spot, spotFilter));
    ...
    >
      <div style={styles.tableCircle}>
        ...
        {facingLinesText.length > 0 && (
            <div style={styles.facingLineBox}>
...
export default function PreflopQuiz() {
  ...
  }, [navPatternId, navCount, navSpotFilter]);

  // patternId / count が変わったら問題セット作り直し
  useEffect(() => {
    const qs = buildRandomSet(patternId, count, null, spotFilter);
    setQuestions(qs);
    setStep(0);
    setSelected(null);
    setLocked(false);
  }, [patternId, count, spotFilter]);

  function regenerate() {
    const prevFirstHand = questions?.[0]?.hero?.hand || null;
    const qs = buildRandomSet(patternId, count, prevFirstHand, spotFilter);
    setQuestions(qs);
    setStep(0);
    setSelected(null);
    setLocked(false);
  ...
  }, [locked, selected, step, questions.length, patternId, count, spotFilter]);
...
const styles = {
  ...
  modalTab: { border: "1px solid rgba(255,255,255,0.15)", color: "#fff", borderRadius: 999, padding: "6px 10px", cursor: "pointer", fontWeight: 800, fontSize: 12 },
  modalTab: {

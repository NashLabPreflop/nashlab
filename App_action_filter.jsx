...
function spotMatchesActionFilter(spot, actionFilter) {
  if (!actionFilter || actionFilter === "all") return true;
  if (!spot || typeof spot.questionBuilder !== "function") return false;

  const [targetPos, targetAction] = String(actionFilter).split("::");
  try {
    const q = spot.questionBuilder("AsKs");
    const pos = normalizeHeroPos(spot.heroPos || q?.pos);
    const options = Array.isArray(q?.options) ? q.options : [];
    return pos === targetPos && options.some((opt) => normalizeActionOption(opt) === targetAction);
  } catch (_err) {
    return false;
  }
}
...
function buildRandomSet(groupId, count, prevFirst = null, spotFilter = "all", heroPosFilter = HERO_POS_FILTER_ALL, actionFilter = "all") {
  ...
  if (isLegacy && actionFilter !== "all" && !spotMatchesActionFilter(group, actionFilter)) return [];
...
function patternMatchesQuizFilters(pattern, spotFilter = "all", heroPosFilter = HERO_POS_FILTER_ALL, actionFilter = "all") {
  ...
  if (rawSpots.length > 0) {
    return rawSpots.some((spot) =>
      spotMatchesFilter(spot, spotFilter) &&
      spotMatchesHeroPos(spot, heroPosFilter) &&
      spotMatchesActionFilter(spot, actionFilter) &&
  ...
  try {
    ...
    if (!spotMatchesActionFilter(pattern, actionFilter)) return false;
    if (heroPosFilter === HERO_POS_FILTER_ALL) return true;
...
function getRandomPatternId(candidatePatternIds = [], prevPatternId = null, spotFilter = "all", heroPosFilter = HERO_POS_FILTER_ALL, actionFilter = "all") {
  ...
  let candidates = (PATTERNS || []).filter((pattern) => {
    ...
    return patternMatchesQuizFilters(pattern, spotFilter, heroPosFilter, actionFilter);
  });
...
function buildRandomQuestionAcrossPatterns(candidatePatternIds = [], prevHand = null, prevPatternId = null, spotFilter = "all", heroPosFilter = HERO_POS_FILTER_ALL, actionFilter = "all") {
  const randomPatternId = getRandomPatternId(candidatePatternIds, prevPatternId, spotFilter, heroPosFilter, actionFilter);
  const questions = buildRandomSet(randomPatternId, 1, prevHand, spotFilter, heroPosFilter, actionFilter);

  // フィルタの都合で空になるケースの保険
  if (questions.length) return questions;

  for (const pattern of PATTERNS || []) {
    if (!patternMatchesQuizFilters(pattern, spotFilter, heroPosFilter, actionFilter)) continue;
    const fallback = buildRandomSet(pattern.id, 1, prevHand, spotFilter, heroPosFilter, actionFilter);
...
export default function PreflopQuiz() {
  ...
  const navCandidatePatternIds = Array.isArray(location.state?.candidatePatternIds)
    ...
    : [];
  const navActionFilter = location.state?.actionFilter || "all";
  ...
  const [actionFilter, setActionFilter] = useState(() => navActionFilter);
  ...
  useEffect(() => {
    ...
    setActionFilter(navActionFilter || "all");
  ...
  useEffect(() => {
    const qs = quizMode === QUIZ_MODE_RANDOM
      ? buildRandomQuestionAcrossPatterns(candidatePatternIds, null, null, spotFilter, heroPosFilter, actionFilter)
      : buildRandomSet(patternId, count, null, spotFilter, heroPosFilter, actionFilter);

    setQuestions(qs);
    setStep(0);
    setSelected(null);
    setLocked(false);
  }, [patternId, count, spotFilter, heroPosFilter, quizMode, candidatePatternIds, actionFilter]);

  function regenerate() {
    const prevFirstHand = questions?.[0]?.hero?.hand || null;
    const prevPatternId = questions?.[0]?.groupId || null;
    const qs = quizMode === QUIZ_MODE_RANDOM
      ? buildRandomQuestionAcrossPatterns(candidatePatternIds, prevFirstHand, prevPatternId, spotFilter, heroPosFilter, actionFilter)
      : buildRandomSet(patternId, count, prevFirstHand, spotFilter, heroPosFilter, actionFilter);
  ...
  function next() {
    ...
    if (quizMode === QUIZ_MODE_RANDOM) {
      ...
      const qs = buildRandomQuestionAcrossPatterns(candidatePatternIds, prevHand, prevPatternId, spotFilter, heroPosFilter, actionFilter);
      setQuestions(qs);
      setStep(0);
      setSelected(null);
      setLocked(false);
  ...
  }, [locked, selected, step, questions.length, patternId, count, spotFilter, heroPosFilter, quizMode, candidatePatternIds, actionFilter]);
